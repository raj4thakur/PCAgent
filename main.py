...
try:
    ...
    st.info("Please make sure all required files (database.py, data_processor.py, analytics.py) are in the same directory.")
...
# Don't cache the components initialization since it takes db as parameter
            ...
            with st.spinner(f"Processing {len(excel_files)} Excel file(s)..."):
                for file_path in excel_files:
                    try:
                        ...
                        if data_processor and data_processor.process_excel_file(file_path):
